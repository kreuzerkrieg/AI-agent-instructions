...
gh api graphql -f query='{
  repository(owner: "scylladb", name: "scylladb") {
...
1. **Analyze each comment** — verify the reviewer's assumptions against actual code before acting.
...
- Verify the body follows the format: Problem → Changes → Issue reference → Backport decision.
...
- `is_object_storage()` made pure virtual, added `filesystem_storage` override
