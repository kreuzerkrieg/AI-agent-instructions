...
The credentials in `~/.aws/credentials` are **temporary STS credentials** (prefix `ASIA...`) 
that expire. The profile `797456418907-DevOpsAccessRole` has an expiry stored in
`x_security_token_expires`.

**Check current expiry:**
```bash
grep x_security_token_expires ~/.aws/credentials
# Entry under [797456418907-DevOpsAccessRole]
```

**The credentials appear to have been generated by `saml2aws`** (the profile naming convention
`AccountID-RoleName` and the `x_security_token_expires` field are saml2aws signatures),
but `saml2aws` is not currently installed on this Fedora machine.

### Option A: AWS IAM Identity Center (SSO) — preferred if ScyllaDB uses it
If ScyllaDB uses AWS SSO (IAM Identity Center), add an SSO profile to `~/.aws/config`:
```ini
[profile devops]
sso_start_url = https://d-XXXXXXXXXX.awsapps.com/start   # fill in ScyllaDB's SSO URL
sso_region = us-east-1
sso_account_id = 797456418907
sso_role_name = DevOpsAccessRole
region = us-east-1
```
Then authenticate:
```bash
...
### Option B: saml2aws (Okta → SAML → AWS)
```bash
sudo dnf install saml2aws   # install on Fedora
saml2aws configure          # set up with Okta IDP URL
saml2aws login              # triggers browser/Okta auth, writes ~/.aws/credentials
```

### Option C: Obtain from the AWS Console (manual, short-lived)
1. Log in to https://console.aws.amazon.com
2. Top-right → your name → "Security credentials" → "Temporary credentials"
   OR: use IAM Identity Center → Account → DevOpsAccessRole → "Command line or programmatic access"
3. Copy the environment variable block (Option 1) or credentials file block (Option 2)
4. Paste into `~/.aws/credentials` under `[797456418907-DevOpsAccessRole]`

### ⚠️ TODO
**Ask a team member** what the ScyllaDB-standard process is for refreshing AWS CLI credentials.
The current credentials expire and there's no automated refresh mechanism set up on this machine.
