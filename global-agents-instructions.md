# AGENTS.md — ScyllaDB AI Coding Agent Global Instructions
...
## SCT (Scylla Cluster Tests) — External Instructions

When the active workspace is the **scylla-cluster-tests** repository (or any task involves SCT), read and follow the instructions in:
```
~/.config/github-copilot/intellij/sct-instructions.md
```
That file contains SCT-specific conventions, architecture notes, analysis workflows, and metric mappings that supplement the repository-level `.github/copilot-instructions.md`. **Always read it at the start of an SCT session** using `read_file` — do not rely on memory from prior conversations.

If the file does not exist yet, notify the user so it can be created.
