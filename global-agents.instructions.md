...
| `scylla/production-cluster-investigation.md` | **Any investigation of a live customer/dbaas cluster** — perf issue, stall, error spike, disk concern, incident triage, ticket referencing a cluster ID | Access prerequisites (WARP), available data sources (Prometheus/Thanos, VictoriaLogs, Grafana, backtrace) with when-to-use, default investigation workflow, Grafana-panel → PromQL mapping, common anti-patterns, reporting format |
