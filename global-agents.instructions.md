...
**Trigger phrase — "suit up":** Ernest often opens a work session by saying **"suit up"** (or "suite up", or "gear up"). Treat it as an explicit instruction to run these mandatory first actions and get ready to work — do not ask for clarification. It is usually followed immediately by a task, code review, or a `$debunk`/`$analyze-ci` request.
